...
class FormularioDoctor:
    ...
    def obtener_por_id(self, id_formulario):
        try:
            consulta = "SELECT * FROM formulario_doctor WHERE id_formulario = %s"
    ...
    def actualizar_diagnostico(self, puntos, diagnostico, id_formulario):
        try:
            cursor = self.conexion.cursor()
            consulta = "UPDATE formulario_doctor SET puntos = %s, diagnostico = %s WHERE id_formulario = %s"
